<?php
/**
 * Contenu de la page d'accueil, découpé en fragments HTML réutilisables.
 *
 * Chaque fragment sert deux usages :
 * 1. Repli statique dans le contenu réel de la page "Accueil" (bloc HTML personnalisé),
 *    utilisé si Elementor est inactif — le site ne doit jamais être vide.
 * 2. Contenu d'un widget "HTML" Elementor par section, pour que la page reste
 *    éditable (réorganiser, dupliquer, remplacer) dans le constructeur Elementor.
 */
...
/**
 * @return array<string,string> Fragments HTML indexés par nom de section.
 */
function diradev_homepage_sections() {
	$services = array();
	...
	if ( empty( $services ) ) {
		$services = array(
			array( 'name' => __( 'Sites Web & Applications', 'diradev' ) ),
			array( 'name' => __( 'Marketing Digital', 'diradev' ) ),
			array( 'name' => __( 'Agents IA & Automatisation', 'diradev' ) ),
			array( 'name' => __( 'Sourcing Produit International', 'diradev' ) ),
		);
	}

	$booking_url = diradev_booking_url();
	$sections    = array();
		...
		<div class="dr-container dr-hero-inner">
			<div class="dr-hero-copy dr-reveal">
				...
				<div class="dr-hero-actions">
					<a class="dr-btn dr-btn-primary" href="<?php echo esc_url( $booking_url ); ?>"><?php esc_html_e( 'Démarrer un projet', 'diradev' ); ?></a>
	...
	<section class="dr-section">
		<div class="dr-container dr-mission">
			...
			<div class="dr-mission-panels dr-reveal">
				<div class="dr-mission-panel a"><span class="dr-mission-tag"><?php esc_html_e( 'Développement Web & App', 'diradev' ); ?></span></div>
				<div class="dr-mission-panel b"><?php echo diradev_icon( 'robot' ); // phpcs:ignore ?><span class="dr-mission-tag"><?php esc_html_e( 'Automatisation IA', 'diradev' ); ?></span></div>
			</div>
		</div>
	</section>
	<?php
	$sections['mission'] = ob_get_clean();
			...
			<div class="dr-services-layout">
				<div class="dr-services-list dr-reveal">
					<?php foreach ( $services as $i => $service ) : ?>
						<a class="dr-service-row" href="<?php echo esc_url( $booking_url ); ?>">
				...
				<div class="dr-services-cards dr-reveal">
					<a class="dr-service-callout light" href="<?php echo esc_url( $booking_url ); ?>">
						<div><h4><?php esc_html_e( 'Une idée de projet ?', 'diradev' ); ?></h4><p><?php esc_html_e( 'Parlons de vos objectifs et voyons comment DiraDev peut vous aider à les atteindre.', 'diradev' ); ?></p></div>
						<span class="dr-service-callout-icon"><?php echo diradev_icon( 'arrow-diag' ); // phpcs:ignore ?></span>
					</a>
					<a class="dr-service-callout accent" href="<?php echo esc_url( $booking_url ); ?>">
						<div><h4><?php esc_html_e( "Besoin d'un agent IA ?", 'diradev' ); ?></h4><p><?php esc_html_e( 'Automatisez vos tâches répétitives et gagnez un temps précieux au quotidien.', 'diradev' ); ?></p></div>
						<span class="dr-service-callout-icon"><?php echo diradev_icon( 'arrow-diag' ); // phpcs:ignore ?></span>
					</a>
				</div>
			</div>
	...
	<?php
	$sections['services'] = ob_get_clean();
	...
	<section class="dr-section" id="process">
		<div class="dr-container">
			...
			<div class="dr-process-grid">
				<div class="dr-process-card dr-reveal"><div class="dr-process-num">01</div><h4><?php esc_html_e( 'Découverte', 'diradev' ); ?></h4><p><?php esc_html_e( 'On échange sur vos objectifs, vos contraintes et vos priorités.', 'diradev' ); ?></p></div>
				<div class="dr-process-card dr-reveal"><div class="dr-process-num">02</div><h4><?php esc_html_e( 'Stratégie', 'diradev' ); ?></h4><p><?php esc_html_e( 'Nous définissons la solution et la feuille de route adaptées.', 'diradev' ); ?></p></div>
				<div class="dr-process-card dr-reveal"><div class="dr-process-num">03</div><h4><?php esc_html_e( 'Réalisation', 'diradev' ); ?></h4><p><?php esc_html_e( 'Développement, création ou automatisation, avec des points réguliers.', 'diradev' ); ?></p></div>
				<div class="dr-process-card dr-reveal"><div class="dr-process-num">04</div><h4><?php esc_html_e( 'Suivi', 'diradev' ); ?></h4><p><?php esc_html_e( 'On reste à vos côtés pour ajuster et faire évoluer votre solution.', 'diradev' ); ?></p></div>
			</div>
		</div>
	</section>
	<?php
	$sections['process'] = ob_get_clean();
	...
	<div class="dr-container" id="contact">
		<div class="dr-cta dr-reveal">
			...
			<div class="dr-cta-content">
				...
				<a class="dr-btn dr-btn-dark" href="<?php echo esc_url( $booking_url ); ?>"><?php esc_html_e( 'Démarrer un projet', 'diradev' ); ?> <?php echo diradev_icon( 'arrow' ); // phpcs:ignore ?></a>
			</div>
		</div>
	</div>
	<?php
	$sections['cta'] = ob_get_clean();
